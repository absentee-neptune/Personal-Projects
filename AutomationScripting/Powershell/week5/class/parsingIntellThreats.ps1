...
Select-String -Path $input_paths -Pattern $regex_drop -AllMatches | % { $_.Matches } | % { $_.Value } | sort | Get-Unique -AsString | Out-File -FilePath "ips-bad.txt"
...
#TASK: access-list 1 deny host 1.1.1.2
(Get-Content -Path ".\ips-bad.txt") | % { $_ -replace "^", "access-list deny host " } `
#| Set-Content -Path ".\ips-bad-iptables.bash"